...
function checkWin(board, rows, cols, winLen = 4) {
...
export class Caro4Game extends GameEngine {
  ...
  applyUserAction(state, action) {
    ...
    if (action.type === 'MOVE') {
      ...
      const { row, col } = action.payload
      s.cursor = { row: Math.max(0, Math.min(s.rows-1, row)), col: Math.max(0, Math.min(s.cols-1, col)) }
